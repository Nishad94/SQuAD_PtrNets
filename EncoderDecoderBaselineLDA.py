...
class BasicS2S(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim,
        ...
        self.decoder = nn.LSTM(3*hidden_dim,hidden_dim)
        self.linearLDa = nn.Linear(300, hidden_dim)
    ...
    def forward(self, inputs, questions, inputs_text, questions_text):
        ...
        ldaVec = torch.zeros(300)
        ldaLinear = self.linearLDa(ldaVec).unsqueeze(0).unsqueeze(0)
        ldaLinear = para_lstm_out.size(0) * [ldaLinear]
        ldaLinear = torch.cat(ldaLinear,dim=0)

        para_quest_concat = torch.cat((para_quest_concat,ldaLinear),dim=2)
